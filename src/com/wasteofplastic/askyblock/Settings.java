...
public class Settings {
	...
	/*
	 * public final static GameType GAMETYPE = GameType.ACIDISLAND; // The spawn
	 * command (Essentials spawn for example) public final static String
	 * SPAWNCOMMAND = "spawn"; // Permission prefix public final static String
	 * PERMPREFIX = "acidisland."; // The island command public final static
	 * String ISLANDCOMMAND = "ai"; // The challenge command public static final
	 * String CHALLENGECOMMAND = "aic"; // Admin command public static final
	 * String ADMINCOMMAND = "acid";
	 */
	...
	public static List<BlockValue> values;

	public static void sortValues() {
		values = new ArrayList<>();
		blockValues.forEach((d, v) -> {
			if (!(d.getItemType().equals(Material.AIR) || d.getItemType().equals(Material.REDSTONE_COMPARATOR_OFF)

					|| d.getItemType().equals(Material.REDSTONE_LAMP_ON)
					|| d.getItemType().equals(Material.REDSTONE_COMPARATOR_ON)
					|| d.getItemType().equals(Material.DIODE_BLOCK_OFF)
					|| d.getItemType().equals(Material.DIODE_BLOCK_ON) || d.getItemType().equals(Material.MELON_STEM)
					|| d.getItemType().equals(Material.REDSTONE_WIRE)
					|| d.getItemType().equals(Material.REDSTONE_TORCH_OFF)
					|| d.getItemType().equals(Material.BURNING_FURNACE) || d.getItemType().equals(Material.LAPIS_ORE)
					|| d.getItemType().equals(Material.IRON_ORE)
					|| d.getItemType().equals(Material.DAYLIGHT_DETECTOR_INVERTED)) && v > 0)
				values.add(new BlockValue(d, v));
		});

		Collections.sort(values, Comparator.comparingInt(BlockValue::getPoint));
	}
}
