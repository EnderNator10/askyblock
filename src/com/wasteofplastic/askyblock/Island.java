...
public class Island {
	...
	public Island(ASkyBlock plugin, String serial, List<String> settingsKey) {
		...
		// Deserialize
		// Format:
		// x:height:z:protection range:island distance:owner UUID: locked:
		// protected
		...
		try {
			...
			if (split.length > 11) {
				try {
					this.hopperLimit = Integer.valueOf(split[11]);
				} catch (Exception e) {
					this.hopperLimit = 20;
				}
				try {
					this.generatorLevel = Integer.valueOf(split[12]);
				} catch (Exception e) {
					this.generatorLevel = 0;
				}
				try {
					this.generatorOre = Boolean.valueOf(split[13]);
				} catch (Exception e) {
					this.generatorOre = true;
				}
				try {
					this.pistonLimit = Integer.valueOf(split[14]);
				} catch (Exception e) {
	...
	public boolean onIsland(Location target) {
		if (world != null) {
			...
			if (target.getWorld().equals(world) || (Settings.createNether && Settings.newNether
					&& ASkyBlock.getNetherWorld() != null && target.getWorld().equals(ASkyBlock.getNetherWorld()))) {
				return target.getBlockX() >= minProtectedX && target.getBlockX() < (minProtectedX + protectionRange)
						&& target.getBlockZ() >= minProtectedZ
	...
	public void setOwner(UUID owner) {
		this.owner = owner;
		// if (owner == null) {
		// Bukkit.getLogger().info("DEBUG: island owner set to null for " +
		// center);
		// }
	}
	...
	public String save() {
		...
		if (isSpawn) {
			...
			if (spawnPoint != null) {
				return center.getBlockX() + ":" + center.getBlockY() + ":" + center.getBlockZ() + ":" + protectionRange
						+ ":" + islandDistance + ":" + ownerString + ":" + locked + ":" + purgeProtected + ":SP:"
						+ Util.getStringLocation(spawnPoint);
			}
			return center.getBlockX() + ":" + center.getBlockY() + ":" + center.getBlockZ() + ":" + protectionRange
		...
		return center.getBlockX() + ":" + center.getBlockY() + ":" + center.getBlockZ() + ":" + protectionRange + ":"
				...
				+ getBiome().toString() + ":" + levelHandicap + ":" + hopperLimit + ":" + generatorLevel + ":"
				+ generatorOre + ":" + pistonLimit;
	}
	...
	public String getSettings() {
		String result = "";
		// Personal island protection settings - serialize enum into 1's and 0's
	...
	public List<UUID> getMembers() {
		...
		if (Settings.createNether && Settings.newNether && ASkyBlock.getNetherWorld() != null) {
			result.addAll(
					CoopPlay.getInstance().getCoopPlayers(center.toVector().toLocation(ASkyBlock.getNetherWorld())));
		}
		if (owner == null) {
			return result;
		}
	...
	/**
	 * @return Provides count of villagers within the protected island
	 *         boundaries
	 */
	...
	/**
	 * @return number of piston on the island
	 */
	public int getPistonCount() {
		tileEntityCount.clear();
		int result = 0;
		for (int cX = getMinProtectedX() / 16; cX <= (getMinProtectedX() + getProtectionSize() - 1) / 16; cX++) {
			for (int cZ = getMinProtectedZ() / 16; cZ <= (getMinProtectedZ() + getProtectionSize() - 1) / 16; cZ++) {
	 ...
	 * @param world
	 *            - world to check
	 * @return count of how many tile entities of type mat are on the island at
	 *         last count. Counts are done when a player places a tile entity.
	 */
	public int getTileEntityCount(Material material, World world) {
		...
		for (int x = getMinProtectedX() / 16; x <= (getMinProtectedX() + getProtectionSize() - 1) / 16; x++) {
			for (int z = getMinProtectedZ() / 16; z <= (getMinProtectedZ() + getProtectionSize() - 1) / 16; z++) {
				...
				for (Entity holder : world.getChunkAt(x, z).getEntities()) {
					// plugin.getLogger().info("DEBUG: entity: " +
					// holder.getType());
		...
		// In 1.7, the entity is present before it is cancelled and so gets
		// counted.
	 ...
	 * 
	 * @param settings
	 *            - string of 0 and 1's that define the settings for the island
	 * @param settingsKey
	 *            - a list showing the order and what flags each digit refers to
	 */
	...
	/**
	 * @return the generatorOre
	 */
	public boolean isGeneratorOre() {
		return generatorOre;
	}

	/**
	 * @param generatorOre
	 *            the generatorOre to set
	 */
	public void setGeneratorOre(boolean generatorOre) {
		this.generatorOre = generatorOre;
	}

	/**
	 * @return the pistonLimit
	 */
