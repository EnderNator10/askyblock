...
public class IslandGuard extends ZUtils implements Listener {
	...
	public void onHorseInventoryClick(InventoryClickEvent event) {
		...
		if (event.getInventory().getHolder() instanceof Animals) {
			if (actionAllowed((Player) event.getWhoClicked(), event.getWhoClicked().getLocation(),
					SettingsFlag.HORSE_INVENTORY)) {
				return;
			}
			// Elsewhere - not allowed
			((Player) event.getWhoClicked()).sendActionBar(
	...
	public void onVehicleDamageEvent(VehicleDamageEvent e) {
		...
		if (inWorld(e.getVehicle())) {
			...
			// Not allowed
			p.sendActionBar(ChatColor.RED + plugin.myLocale(p.getUniqueId()).islandProtected);
			p.sendActionBar(ChatColor.RED + plugin.myLocale(p.getUniqueId()).islandProtected);
			e.setCancelled(true);
		}
	}
	...
	public void onVehicleMove(final VehicleMoveEvent e) {
		...
		if (islandTo != null && islandFrom == null) {
			...
			if (islandTo.getOwner() != null && (islandTo.isLocked()
					|| plugin.getPlayers().isBanned(islandTo.getOwner(), player.getUniqueId()))) {
				Util.sendMessage(player, ChatColor.RED + plugin.myLocale(player.getUniqueId()).lockIslandLocked);
			}
			if (islandTo.isSpawn()) {
			...
			if (islandFrom.isSpawn()) {
				...
				if (islandFrom.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
					Util.sendEnterExit(player, plugin.myLocale(player.getUniqueId()).lockNowLeaving.replace("[name]",
			...
			if (islandTo.isSpawn()) {
				...
				if (islandTo.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
					Util.sendEnterExit(player, plugin.myLocale(player.getUniqueId()).lockNowEntering.replace("[name]",
		...
		if (islandTo != null && islandFrom == null) {
			...
			if (islandTo.isSpawn()) {
				...
				if (islandTo.getOwner() != null
					...
					if (islandTo.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
						...
						// plugin.myLocale(e.getPlayer().getUniqueId()).lockNowEntering.replace("[name]",
						// plugin.getGrid().getIslandName(islandTo.getOwner())));

						actionMessage(e.getPlayer(), Message.ISLAND_ENTER,
								plugin.getGrid().getIslandName(islandTo.getOwner()));
			...
			if (islandFrom.isSpawn()) {
				...
				if (islandFrom.getOwner() != null
					...
					if (islandFrom.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
						// actionMessage(e.getPlayer(), Message.ISLAND_LEAVE,
						// plugin.getGrid().getIslandName(islandFrom.getOwner()));
						actionMessage(e.getPlayer(), Message.ISLAND_LEAVE,
								plugin.getGrid().getIslandName(islandFrom.getOwner()));
					}
	...
	public void onEntityDamage(final EntityDamageByEntityEvent e) {
		...
		if (island != null && island.isSpawn() && e.getDamager().getType().equals(EntityType.CREEPER)
				&& island.getIgsFlag(SettingsFlag.CREEPER_PAIN)) {
			return;
		}
		...
		if (e.getEntity() instanceof ItemFrame || e.getEntityType().toString().endsWith("STAND")) {
			...
			// Else not allowed
			attacker.sendActionBar(ChatColor.RED + plugin.myLocale(attacker.getUniqueId()).islandProtected);
			if (flamingArrow)
				...
				if (!Settings.allowCreeperGriefing && e.getEntity() instanceof Creeper) {
					for (ItemStack holding : Util.getPlayerInHandItems(attacker)) {
						if (holding.getType().equals(Material.FLINT_AND_STEEL)) {
							// Save this creeper for later when any damage
							// caused by its explosion will be nullified
		...
		if (e.getEntity() instanceof Animals || e.getEntity() instanceof IronGolem || e.getEntity() instanceof Snowman
			...
			if (island != null && (island.getIgsFlag(SettingsFlag.HURT_MOBS)
					|| island.getMembers().contains(attacker.getUniqueId()))) {
				return;
			}
			if (DEBUG)
		...
		boolean pvp = false;
		if ((inNether && island != null && island.getIgsFlag(SettingsFlag.NETHER_PVP)
				|| (!inNether && island != null && island.getIgsFlag(SettingsFlag.PVP)))) {
			if (DEBUG)
			...
			for (ItemStack item : Util.getPlayerInHandItems(e.getPlayer())) {
				if (item.getType().equals(Material.MOB_SPAWNER) && item.hasItemMeta() && item.getItemMeta().hasLore()) {
					...
					if (!lore.isEmpty()) {
						...
						// Spawner type not found - do anything : it may be
						// another plugin's spawner
					}
				}
			}
		}
	...
	public void onLeashUse(final PlayerLeashEntityEvent e) {
		...
		if (inWorld(e.getEntity())) {
			if (e.getPlayer() != null) {
				...
				player.sendActionBar(ChatColor.RED + plugin.myLocale(player.getUniqueId()).islandProtected);
				e.setCancelled(true);
		...
		if (inWorld(e.getPlayer())) {
			...
			if (e.getBlockClicked() != null) {
				...
				if (actionAllowed(p, dumpBlock.getLocation(), SettingsFlag.BUCKET)) {
					// Check if biome is Nether and then allow water placement
					// but fizz the water
						...
						if (plugin.getServer().getVersion().contains("(MC: 1.8")
							...
							if (e.getPlayer().getItemInHand().getType().equals(Material.WATER_BUCKET)) {
								...
								Util.sendMessage(e.getPlayer(),
										ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).biomeSet
	...
	public void onNetherDispenser(final BlockDispenseEvent e) {
		...
		if (e.getBlock().getWorld().getEnvironment().equals(Environment.NETHER) || !inWorld(e.getBlock().getLocation())
	...
	public void onBucketFill(final PlayerBucketFillEvent e) {
		...
		if (inWorld(e.getPlayer())) {
			...
			if (island != null) {
				...
				if (island.getIgsFlag(SettingsFlag.COLLECT_LAVA)
						&& e.getItemStack().getType().equals(Material.LAVA_BUCKET)) {
					return;
				}
				if (island.getIgsFlag(SettingsFlag.COLLECT_WATER)
						&& e.getItemStack().getType().equals(Material.WATER_BUCKET)) {
					return;
				}
				if (island.getIgsFlag(SettingsFlag.MILKING)
						&& e.getItemStack().getType().equals(Material.MILK_BUCKET)) {
					return;
				}
	...
	public void onPlayerInteract(final PlayerInteractEvent e) {
		...
		if (e.getClickedBlock() != null) {
			...
			try {
				...
				// To catch at block iterator exceptions that can happen in the
				// void or at the very top of blocks
				if (DEBUG) {
				...
				try {
					...
					if (p.isSplash()) {
						...
						if (island == null) {
							...
							if ((inNether && island.getIgsFlag(SettingsFlag.NETHER_PVP)
									|| (!inNether && island.getIgsFlag(SettingsFlag.PVP)))) {
								return;
							}
						}
						// Not allowed
	...
	public void onFishing(final PlayerFishEvent e) {
		...
		if (e.getCaught() instanceof Player) {
			...
			if (island == null
				...
				Util.sendMessage(e.getPlayer(),
						ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).targetInNoPVPArea);
				e.setCancelled(true);
				e.getHook().remove();
				return;
			}
			if (island != null && ((e.getCaught().getWorld().getEnvironment().equals(Environment.NORMAL)
		...
		if (!plugin.getGrid().playerIsOnIsland(e.getPlayer())) {
			if (e.getCaught() instanceof Animals) {
				...
				if (island != null) {
					if ((!island.getIgsFlag(SettingsFlag.HURT_MOBS)
			...
			if (e.getCaught() instanceof Monster || e.getCaught() instanceof Squid || e.getCaught() instanceof Slime) {
				...
				if (island != null) {
					if ((!island.getIgsFlag(SettingsFlag.HURT_MONSTERS)
							&& !island.getMembers().contains(p.getUniqueId()))) {
						e.getPlayer().sendActionBar(
								ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).islandProtected);
						e.setCancelled(true);
	...
	public void onPlayerHitEntity(PlayerInteractEntityEvent e) {
		...
		/*
		 * Leashes are deal with mostly using the PlayerLeashEvent and
		 * PlayerUnleashEvent however, skeleton and zombie horses cannot be
		 * leashed, so those should be exempted
		 */
		if (Util.playerIsHolding(p, Material.LEASH) && e.getRightClicked() != null) {
			...
			if (e.getRightClicked() instanceof Horse) {
				...
				if (horse.getVariant().equals(Variant.SKELETON_HORSE)
						|| horse.getVariant().equals(Variant.UNDEAD_HORSE)) {
					if (DEBUG)
		...
		if (!plugin.getGrid().playerIsOnIsland(e.getPlayer())) {
			...
			if (e.getRightClicked() != null && e.getRightClicked().getType().equals(EntityType.VILLAGER)) {
				if (island != null) {
					if (DEBUG) {
						...
						plugin.getLogger()
								.info("DEBUG: villager trading is " + island.getIgsFlag(SettingsFlag.VILLAGER_TRADING));
					}
					if ((!island.getIgsFlag(SettingsFlag.VILLAGER_TRADING)
							&& !island.getMembers().contains(p.getUniqueId()))) {
						e.getPlayer().sendActionBar(
								ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).islandProtected);
						e.setCancelled(true);
			...
			if (Util.playerIsHolding(p, Material.NAME_TAG) || Util.playerIsHolding(p, Material.INK_SACK)) {
				e.getPlayer()
						.sendActionBar(ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).islandProtected);
				e.setCancelled(true);
				e.getPlayer().updateInventory();
				return;
			}
		...
		if (e.getCause() != null) {
			...
			case FLINT_AND_STEEL:
				...
				if (DEBUG) {
					...
					// plugin.getLogger().info("DEBUG: target block = " +
					// e.getPlayer().getTargetBlock(transparent, 10));
				}
				// Check if this is allowed
				if (e.getPlayer() != null && (e.getPlayer().isOp()
						|| VaultHelper.checkPerm(e.getPlayer(), Settings.PERMPREFIX + "mod.bypass"))) {
					return;
				}
				if (!actionAllowed(e.getBlock().getLocation(), SettingsFlag.FIRE)) {
					if (DEBUG)
	...
	public void onPlateStep(PlayerInteractEvent e) {
		...
		if (!onPlate.containsKey(playerUUID)) {
			e.getPlayer().sendActionBar(ChatColor.RED + plugin.myLocale(playerUUID).islandProtected);
					...
					if (!plugin.getGrid().locationIsAtHome(shooter, true, e.getBlock().getLocation())) {
						...
						if (!tntBlocks.contains(e.getBlock().getLocation())) {
							shooter.sendActionBar(
									ChatColor.RED + plugin.myLocale(shooter.getUniqueId()).islandProtected);
	...
	public void onEvent(BlockPistonExtendEvent event) {
		...
		/*
		 * JAVA 8 if
		 * (event.getBlocks()..stream().anyMatch(it->it.getType()==Material.TNT)
		 * ) event.setCancelled(true);
		 */
	}
	...
	/**
	 * Checks for splash damage. If there is any to any affected entity and it's
	 * not allowed, it won't work on any of them.
	 * 
	 * @param e
	 *            - event
	 */
	...
	public void onSplashPotionSplash(final PotionSplashEvent e) {
		if (DEBUG) {
			...
			// plugin.getLogger().info("splash hit entity = " +
			// e.getHitEntity());
		}
			...
			for (LivingEntity entity : e.getAffectedEntities()) {
				...
				if (entity instanceof Player) {
					if (pvp) {
						...
						Util.sendMessage(attacker,
								ChatColor.RED + plugin.myLocale(attacker.getUniqueId()).targetInNoPVPArea);
						e.setCancelled(true);
						return;
					}
				}
